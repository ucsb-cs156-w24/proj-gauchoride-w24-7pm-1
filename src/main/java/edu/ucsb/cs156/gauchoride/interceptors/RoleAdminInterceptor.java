...
@Component
public class RoleAdminInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if (authentication.getClass() == OAuth2AuthenticationToken.class) {
            ...
            if (optionalUser.isPresent()) {
                ...
                revisedAuthorities = authorities.stream().filter(auth -> !auth.equals("ROLE_ADMIN")).collect(Collectors.toSet());
                if (user.getAdmin()){
